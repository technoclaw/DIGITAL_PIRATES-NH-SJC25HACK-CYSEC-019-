...
// Server-side proxy to n8n webhook for threat intelligence analysis.
// Configure the webhook URL and optional auth in server env vars:
// N8N_THREAT_INTEL_WEBHOOK - full webhook URL
...
export async function POST(req: Request) {
  ...
  if (!webhook) return NextResponse.json({ error: 'N8N_THREAT_INTEL_WEBHOOK not configured' }, { status: 500 })
  ...
  try {
    ...
    console.log('[Proxy] Forwarding threat intel request to n8n webhook:', webhook)
