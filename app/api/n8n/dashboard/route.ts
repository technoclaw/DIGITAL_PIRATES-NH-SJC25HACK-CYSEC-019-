...
// Server-side proxy to n8n webhook for dashboard/home page updates.
// Configure the webhook URL and optional auth in server env vars:
// N8N_DASHBOARD_WEBHOOK - full webhook URL
...
export async function POST(req: Request) {
  ...
  if (!webhook) return NextResponse.json({ error: 'N8N_DASHBOARD_WEBHOOK not configured' }, { status: 500 })
  ...
  try {
    ...
    console.log('[Proxy] Forwarding dashboard request to n8n webhook:', webhook)
...
export async function GET() {
  ...
  try {
    // GET request for dashboard summary/status
    const res = await fetch(webhook, { method: 'GET', headers: { ...getAuthHeaders() } })
    const text = await res.text()
    
    console.log('[Proxy] Dashboard GET response status:', res.status)
